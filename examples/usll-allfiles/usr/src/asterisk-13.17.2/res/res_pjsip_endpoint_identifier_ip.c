...
static int header_identify_match_check(void *obj, void *arg, int flags)
{
	...
	char *c_header;
	char *c_value;
//DBW 20180227  Define uri and toheader
	pjsip_sip_uri *sip_to;
...
/* DBW: 2018-02-16 Modified to find the value contained in the string rather than exact match 
...
*/
	...
	if (strcmp(c_header,"To") == 0) {

		pj_header_value = pj_str(c_value);
		sip_to = (pjsip_sip_uri *) pjsip_uri_get_uri (rdata->msg_info.to->uri);
	
//ast_debug(3, "HERE  ONE '%s' TWO '%s' Result %i\n",sip_to->host, pj_header_value, pj_strstr(&ip_to->host, &pj_header_value));
		if (pj_strcmp(&sip_to->host, &pj_header_value)) {
			ast_debug(3, "SIP message contains header '%s' but string '%s' was not found in value '%.*s' for endpoint '%s'\n",
			c_header,
			c_value,
			(int) pj_strlen(&sip_to->host), pj_strbuf(&sip_to->host),
			identify->endpoint_name);
			return 0;
		}
	} else {
